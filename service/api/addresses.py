...
def create_address(payload, person_id):
    ...
    # ahead and create with no additional logic.
        ...
        prev_address_segment = person.address_segments[0]
        prev_start_date = prev_address_segment.start_date
        new_start_date = payload.get("start_date")

        if new_start_date > prev_start_date:
            setattr(prev_address_segment, "end_date", new_start_date)

            db.session.add(prev_address_segment)
            db.session.commit()
            db.session.refresh(prev_address_segment)
            ...
            abort(404, description="new address start date must be greater than previous address start date")
